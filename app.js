...
(function(){
  ...
  function renderTopBar(){ const logo = $('teamLogo'); const nameBar = $('teamNameBar'); const dStand = $('topDriverStandings'); const cStand = $('topConstructorStandings'); const playerTeam = state.teams.find(t=>t.id===state.playerTeamId) || null; if(logo){ if(playerTeam){ // make initials
        const initials = getInitials(playerTeam.name); logo.textContent = initials; logo.style.background = `linear-gradient(135deg, rgba(225,29,72,0.15), rgba(14,165,233,0.06))`; nameBar.textContent = playerTeam.name; } else { logo.textContent = ''; nameBar.textContent = ''; } }
    // Top drivers (3)
  ...
  function showStartScreen(){ document.body.classList.add('noscroll'); $('startScreen').classList.remove('hidden'); $('appWrapper').classList.add('hidden'); renderTeamList(); const first = document.querySelector('#teamList .team-card'); if(first) first.focus(); }
  function hideStartScreen(){ document.body.classList.remove('noscroll'); $('startScreen').classList.add('hidden'); $('appWrapper').classList.remove('hidden'); const next = document.getElementById('nextRace'); if(next) next.focus(); }

  function boot(){ try{ localStorage.removeItem('miniF1'); }catch(e){} newSeason(); wire(); showStartScreen(); }
